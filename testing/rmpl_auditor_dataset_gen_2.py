...
def make_action_gate_example() -> dict:
    ...
    return {"task": "action_gate", "input": {"action": action}, "label": "CONFIRM_REQUIRED" if requires else "ALLOW", "reason": "Destructive/external action." if requires else "Safe read-only action.", "generated_at": iso_now()}
